        ...
        <div
          ...
          <button
            ...
            <div class="flex items-center gap-2 min-w-0">
              <Icon name="i-lucide-wallet" class="size-4 text-amber-400 shrink-0" />
              <span class="text-[13px] font-medium text-white truncate">{{ item.walletName }}</span>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              <span class="text-[13px] text-white/60">{{ item.network }}</span>
              <Icon
          ...
          <div v-show="mobileExpandedId === item.id" class="border-t border-white/6 p-3 space-y-2">
            <div class="flex justify-between text-[12px]">
              <span class="text-white/40">{{ $t('crypto.currency') }}</span>
              <span class="text-white/80 font-medium">{{ item.currency }}</span>
            </div>
            <div class="flex justify-between text-[12px]">
              <span class="text-white/40">{{ $t('crypto.address') }}</span>
              <span class="text-white/80 font-mono text-[11px] break-all text-right max-w-50">{{ item.address }}</span>
            </div>
            <div class="flex justify-between text-[12px]">
            ...
            <div class="flex items-center justify-end gap-2 mt-1">
              <UButton size="xs" color="error" variant="soft" icon="i-lucide-trash-2" class="cursor-pointer" @click="onDeleteCryptoAddress(item.id)">
                {{ $t('common.delete') }}
              </UButton>
            </div>
          </div>
