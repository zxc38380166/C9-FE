...
<script setup lang="ts">
  ...
  const channelOptions = computed<SelectItem[]>(() => {
    ...
    return channels.value
      .filter((c: VendorChannel) => {
        ...
        const methodMatch = !c.paymentMethods?.length || c.paymentMethods.includes('fiat');
  ...
  const selectedBankCard = ref<string | undefined>(undefined);

  const bankCardOptions = computed<SelectItem[]>(() => {
